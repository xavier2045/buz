        ...
        <div class="filter-content" v-if="expandedFilters['commercial-use']">
          ...
          <div v-if="filterSelections['commercial-use'] === 'yes' && showCommercialUpload" class="file-upload-section">
            <FileUpload
              ...
              variant="green"
              description="选择商业使用必须上传原文件"
              @update:modelValue="(file) => handleCommercialUpload(file)"
            />
          </div>
          ...
          <div v-if="filterSelections['commercial-use'] === 'no' && showNonCommercialUpload" class="file-upload-section">
            <FileUpload
              ...
              variant="purple"
              description="选择非商业使用可选上传原文件"
              @update:modelValue="(file) => handleNonCommercialUpload(file)"
            />
          </div>
        ...
        <div class="filter-content" v-if="expandedFilters['licensing-fee']">
          ...
          <div class="filter-options">
            ...
            <div class="fee-input-container">
              ...
              <div class="fee-input-wrapper">
                <input type="number" v-model="revenueShare" class="fee-input" placeholder="0" min="0" max="100">
        ...
        <div class="filter-content" v-if="expandedFilters['remixing']">
          ...
          <div v-if="filterSelections['remixing'] === 'yes' && showRemixUpload" class="file-upload-section">
            <FileUpload
              v-model="remixFile"
              variant="red"
              description="选择商业混音必须上传原文件"
              @update:modelValue="(file) => handleRemixUpload(file)"
            />
          </div>
...
// 观察商业使用和许可费用的变化
watch([() => props.filterSelections['commercial-use'], licensePrice, revenueShare], ([commercialUse, price, share]) => {
  // 只有当选择了商业使用为'yes'时才验证许可费用
  if (commercialUse === 'yes') {
    // 检查是否设置了价格或分成
    const isValid = price > 0 || share > 0;
    ...
    // 通知父组件验证状态
    emit('validate-licensing-fee', isValid);
...
const handleFileUpload = (fileType: string, file: File | null) => {
  ...
  } else if (fileType === 'nonCommercial') {
    nonCommercialFile.value = file;
  } else if (fileType === 'remix') {
    remixFile.value = file;
  }
  
  // 触发文件上传事件，传递给父组件
  emit('file-upload', {
    isCommercial: fileType === 'commercial' || fileType === 'remix',
    file: file as File
  });
};

const handleCommercialUpload = (file: File | null) => {
  handleFileUpload('commercial', file);
};

const handleNonCommercialUpload = (file: File | null) => {
  handleFileUpload('nonCommercial', file);
};

const handleRemixUpload = (file: File | null) => {
  handleFileUpload('remix', file);
};
...
const triggerFileInput = (isCommercial: boolean) => {
  ...
  fileInput.onchange = (event) => {
    ...
    if (input.files && input.files.length > 0) {
      ...
      if (isCommercial) {
        commercialFile.value = selectedFile;
      } else {
        nonCommercialFile.value = selectedFile;
      }
      handleFileUpload(isCommercial, selectedFile);
    }
  };
